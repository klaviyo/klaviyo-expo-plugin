...
|`ios.automaticPushOpenTracking`| boolean | optional | Writes the `automatic_push_open_tracking` key to the app's `Info.plist`. Set to `true` to opt in to the native iOS SDK's automatic push open tracking (native default is OFF). Omitting this prop removes the key so the native default applies. Default: `undefined` (key not written) |
|`ios.automaticPushTokenForwarding`| boolean | optional | Writes the `automatic_push_token_forwarding` key to the app's `Info.plist`. Set to `true` to opt in to the native iOS SDK's automatic push token forwarding (native default is OFF). Omitting this prop removes the key so the native default applies. Default: `undefined` (key not written) |
