 ...
 *
 * Plugin 1.0.0 recorded this plist with an ABSOLUTE path and never got it into a build phase
 * (its buildPhaseObject call had the arguments transposed, so the add silently no-opped). On
 * an additive upgrade - `expo prebuild --no-clean` over an ios/ generated by 1.0.0 -
 * hasFile(relative) does not match that legacy entry, so the project would keep a stale
 * machine-specific reference next to the new working one. The build still succeeds, because
 * only the new reference is in the resources phase, but the dead entry points at another
 * developer's home directory and is confusing at best.
 *
...
function pruneLegacyPlistReferences(
  ...
  const staleRefIds = Object.keys(fileRefs)
  ...
  for (const refId of staleRefIds) {
    // Any PBXBuildFile that wraps this reference, plus its entry in every build phase.
    const staleBuildFileIds = Object.keys(buildFiles)
      .filter(key => !key.endsWith('_comment'))
      .filter(key => unquote(buildFiles[key]?.fileRef) === refId);

    for (const buildFileId of staleBuildFileIds) {
      for (const phaseName of ['PBXResourcesBuildPhase', 'PBXSourcesBuildPhase']) {
        const phases = objects[phaseName] ?? {};
      ...
      delete buildFiles[`${buildFileId}_comment`];
    }

    // Drop it from whichever group lists it as a child.
    ...
    delete fileRefs[refId];
    delete fileRefs[`${refId}_comment`];
  }

  return staleRefIds.length;
}
...
const withKlaviyoPluginConfigurationPlist: ConfigPlugin = config => {
  return withXcodeProject(config, async (config) => {
    ...
    if (fs.existsSync(srcPlistPath)) {
      ...
      // Prune before the hasFile check: a 1.0.0-generated project holds an ABSOLUTE
      // reference that hasFile(relative) does not match, so without this an additive
      // upgrade keeps the stale machine-specific entry alongside the new one.
      const pruned = pruneLegacyPlistReferences(xcodeProject, relativePlistPath);
      if (pruned > 0) {
        KlaviyoLog.log(
          `Removed ${pruned} legacy klaviyo-plugin-configuration.plist reference(s) that used an absolute path`
        );
      }
