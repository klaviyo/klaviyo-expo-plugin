...
describe('withKlaviyoPluginConfigurationPlist (real xcode project)', () => {
  ...
  it('warns about a legacy absolute reference and removes nothing', async () => {
    // 1.0.0 wrote an absolute path and never got the plist into a build phase. We warn
    // rather than delete: a misidentified reference would be a file registration the
    // consumer owns, and the thing being fixed is a red icon that cannot fail a build.
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const xcode = require('xcode');
    const project = xcode.project(FIXTURE).parseSync();
    ...
    // Still there - we do not touch it.
    expect(project.pbxFileReferenceSection()[legacyId]).toBeDefined();

    const warnings = (KlaviyoLog.warn as jest.Mock).mock.calls.map((c: any[]) => String(c[0]));
    expect(warnings.some((w) => w.includes(legacyPath))).toBe(true);
    expect(warnings.some((w) => w.includes('expo prebuild --clean'))).toBe(true);
  });

  it('does not warn about a same-named plist the app owns elsewhere', async () => {
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const xcode = require('xcode');
    const project = xcode.project(FIXTURE).parseSync();

    project.pbxFileReferenceSection()['BBBBBBBBBBBBBBBBBBBBBBBB'] = {
      isa: 'PBXFileReference',
      name: '"klaviyo-plugin-configuration.plist"',
      path: '"/shared/Config/klaviyo-plugin-configuration.plist"',
      sourceTree: '"<group>"',
    };
    ...
    const warnings = (KlaviyoLog.warn as jest.Mock).mock.calls.map((c: any[]) => String(c[0]));
    expect(warnings.some((w) => w.includes('/shared/Config'))).toBe(false);
  });
});
